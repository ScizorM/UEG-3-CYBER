...
system.runInterval((runInt) => {
    ...
    world.getAllPlayers().forEach(player => {
        ...
        // if(mb85Player > 0){
        //     mb85ParryTimer.addScore(player,-1)
        // }
        // else{
        //     player.removeTag("parrying")
        // }
        ...
        world.getDimension("Overworld").runCommand("execute as @a run execute unless entity @s[hasitem={item=sm:mb85_greatsword,location=slot.weapon.mainhand}] run tag @s remove mb85_greatsword")

        if(player.hasTag("mb85_greatsword")){
            player.addTag("parrying")
        }
        else{
            player.removeTag("parrying")
        }  
...
function ApplyMeleeKnockback(attacker,target,horizontalStrength,verticalStrength,push){
    if(!target.hasTag("parrying")){
        if(push){
            target.applyKnockback(attacker.getViewDirection().x, attacker.getViewDirection().z, horizontalStrength, verticalStrength)
        }
        else{
            target.applyKnockback(-attacker.getViewDirection().x, -attacker.getViewDirection().z, horizontalStrength, verticalStrength)
        }
    ...
    var allEntities = world.getDimension("overworld").getEntities({maxDistance: 4, location: player.location,tags: ["hit"], excludeFamilies:["npc","creeper"]}).forEach(entity => {
        ...
        if(entity.hasTag("hit_bloomfan")){
            if(!entity.hasTag("kbApplied")){
                ApplyMeleeKnockback(player,entity,8,0.3,true)
        ...
        if(entity.hasTag("hit_repulsion")){
            if(!entity.hasTag("kbApplied")){
                ApplyMeleeKnockback(player,entity,5,0.3,true)
        ...
        if(entity.hasTag("hit_greatsword")){
            if(!entity.hasTag("kbApplied")){
                ApplyMeleeKnockback(player,entity,10,0.5,true)

                entity.addTag("kbApplied")
            }
            
            entity.runCommand("function explosion_crimson")
            entity.runCommand("damage @s 1")
            entity.removeTag("hit")
            entity.removeTag("hit_greatsword")
            entity.removeTag("kbApplied")
        }

        if(entity.hasTag("hit_kusarigama")){
            if(!entity.hasTag("kbApplied")){
                ApplyMeleeKnockback(player,entity,1,0.35,false)
                entity.addTag("kbApplied")
            }
        ...
        if(entity.hasTag("hit_heavenpiercer")){
            if(!entity.hasTag("kbApplied")){
                ApplyMeleeKnockback(player,entity,2,0.3,true)
                entity.addTag("kbApplied")
        ...
        if(entity.hasTag("hit_holic")){
            if(!entity.hasTag("kbApplied")){
                ApplyMeleeKnockback(player,entity,1,0.3,true)
                entity.addTag("kbApplied")
        ...
        if(entity.hasTag("hit_gobbler")){
            if(!entity.hasTag("kbApplied")){
                ...
                ApplyMeleeKnockback(player,entity,2,-1,true)
