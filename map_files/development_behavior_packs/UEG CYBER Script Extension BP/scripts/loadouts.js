...
function giveItems(slotString, selectedSlot, player) {
    ...
    let forcedNu = forcedLoadoutsSB.getScore("nu_enabled")
    let forcedLambda = forcedLoadoutsSB.getScore("lambda_enabled")
    let forcedBoth = forcedLoadoutsSB.getScore("both_enabled")

    if (player.hasTag("team_nu") && (forcedNu == 1 || forcedBoth == 1)) {
        SendDebugMessage("forced loadout for nu")
        for (var i = 0; i < 9; i++) {
            var loadoutSlot = "nu_slot" + (i).toString()
            var currentLoadoutSB = world.scoreboard.getObjective("forced_loadouts")
    ...
    else if (player.hasTag("team_lambda") && (forcedLambda == 1 || forcedBoth == 1)) {
        SendDebugMessage("forced loadout for lambda")
        for (var i = 0; i < 9; i++) {
            var loadoutSlot = "lambda_slot" + (i).toString()
            var currentLoadoutSB = world.scoreboard.getObjective("forced_loadouts")
            var currentLoSlPlayer = currentLoadoutSB.getScore(loadoutSlot)
            var itemName = ingameWeaponIDList[currentLoSlPlayer]
            if (itemName != "checkforteam_landmine" && itemName != "checkforteam_remote") {
