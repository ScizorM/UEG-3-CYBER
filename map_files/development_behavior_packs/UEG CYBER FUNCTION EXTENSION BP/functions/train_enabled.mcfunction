execute unless score game_active game_active matches 1 run /tellraw @a {"rawtext": [{"text": "§e[PVP Announcements] §aThere are now enough players to start a match. Enter the train on the first floor of the lobby (metro) to start."}]}
